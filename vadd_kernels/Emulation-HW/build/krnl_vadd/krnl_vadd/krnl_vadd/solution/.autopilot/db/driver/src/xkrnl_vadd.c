...
void XKrnl_vadd_Set_elements(XKrnl_vadd *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XKrnl_vadd_WriteReg(InstancePtr->Control_BaseAddress, XKRNL_VADD_CONTROL_ADDR_ELEMENTS_DATA, Data);
}

u32 XKrnl_vadd_Get_elements(XKrnl_vadd *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XKrnl_vadd_ReadReg(InstancePtr->Control_BaseAddress, XKRNL_VADD_CONTROL_ADDR_ELEMENTS_DATA);
    return Data;
}
